...
func TestGojiMongoDB(t *testing.T) {
	...
	app.Post("/books", func(ctx iris.Context) {
		addBook(session)
	})

	app.Get("/books/:isbn", func(ctx iris.Context) {
		bookByISBN(session)
	})
	app.Put("/books/:isbn", func(ctx iris.Context) {
		updateBook(session)
	})
	app.Delete("/books/:isbn", func(ctx iris.Context) {
		deleteBook(session)
	})
